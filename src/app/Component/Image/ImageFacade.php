...
class ImageFacade
{
    ...
    public function getPreviewName(string $fileName, ?int $width = null, ?int $height = null): string
    {
        ...
        unset($fileName[0]);
        $newFileName .= '_' . ($width === null ? 'null' : $width) . '_' . ($height === null ? 'null' : $height) . '_' . implode($fileName);
        return $newFileName;
    }

    public function preview(string $fileName, ?int $width = null, ?int $height = null): string
    {
        $previewFile = $this->parameterBag->previewDir . '/' . $this->getPreviewName($fileName, $width === null ? 0 : $width, $height === null ? 0 : $height);
        if(file_exists($previewFile)) {
            return $this->parameterBag->previewWwwDir . '/' . $this->getPreviewName($fileName, $width === null ? 0 : $width, $height === null ? 0 : $height);
        } else {
